...
import { DeleteConfirm, RowEditor } from "@/components/DatabaseEditor";
...
function RowActions({ onEdit, onDelete }: { onEdit: () => void; onDelete: () => void }) {
  return (
    <div className="flex shrink-0 gap-1">
      <button
        onClick={onEdit}
        className="rounded-lg border border-slate-800 px-2.5 py-1 text-xs text-slate-400 transition-colors hover:border-blue-700 hover:text-blue-300"
      >
        {DATABASE_WRITE_COPY.edit}
      </button>
      <button
        onClick={onDelete}
        className="rounded-lg border border-slate-800 px-2.5 py-1 text-xs text-slate-500 transition-colors hover:border-red-800 hover:text-red-300"
      >
...
function StructuralChip({ table, column }: { table: string; column: string }) {
  return (
    <span
      ...
      className="cursor-help rounded-full border border-violet-900 bg-violet-950/40 px-2 py-0.5 text-[10px] font-medium text-violet-300"
    >
        ...
        <Card key={filename} className="!p-4">
          <div className="flex flex-wrap items-start justify-between gap-2">
            <div className="min-w-0">
              <p className="truncate font-medium text-slate-100">{filename}</p>
          ...
          <details className="group mt-3">
            ...
            <div className="mt-2 space-y-2">
              {entry.chunks.map((chunk) => (
                <div
                  key={String(chunk.id)}
                  className="flex items-start justify-between gap-2 rounded-lg border border-slate-800 bg-slate-950/60 p-2.5"
                >
                  <p className="text-xs leading-relaxed text-slate-400">{text(chunk, "text")}</p>
                  <button
                    onClick={() => actions.onEdit(chunk)}
                    className="shrink-0 rounded-lg border border-slate-800 px-2 py-0.5 text-xs text-slate-500 transition-colors hover:border-blue-700 hover:text-blue-300"
                  >
                    {DATABASE_WRITE_COPY.edit}
                  </button>
                </div>
        ...
        <Card key={session} className="!p-4">
          <div className="mb-3 flex flex-wrap items-center justify-between gap-2 border-b border-slate-800 pb-2">
            <p className="text-sm font-medium text-slate-200">
              <span className="text-slate-500">{DATABASE_COPY.memorySession}: </span>
              {session}
            </p>
            <span className="text-xs text-slate-600">{DATABASE_COPY.memoryMessages(turns.length)}</span>
          </div>
                  ...
                  <div
                    className={`max-w-[85%] rounded-2xl px-3.5 py-2 ${
                      mine
                        ...
                        : "rounded-tr-sm border border-violet-900/60 bg-violet-950/30 text-violet-100"
                    }`}
                  >
        ...
        <Card key={String(row.id)} className="!p-4">
          <div className="flex flex-wrap items-center justify-between gap-2">
            <div className="flex flex-wrap items-center gap-2">
              ...
              <span className="text-xs text-slate-600">#{text(row, "id")}</span>
            </div>
            <div className="flex items-center gap-3">
              <span className="text-xs text-slate-600">{when(row)}</span>
          ...
          <details className="group mt-3">
            ...
            <div className="mt-3 space-y-3">
              <div>
                ...
                <p className="max-h-40 overflow-y-auto whitespace-pre-wrap rounded-lg border border-slate-800 bg-slate-950/60 p-2.5 font-mono text-xs leading-relaxed text-slate-500">
              ...
              <div>
                ...
                <p className="max-h-72 overflow-y-auto whitespace-pre-wrap text-sm leading-relaxed text-slate-300">
                  {text(row, "result")}
                </p>
              </div>
            </div>
        ...
        return (
          <Card key={text(row, "question")} className="!p-4">
            ...
            <div className="mt-2.5 flex flex-wrap items-center gap-2">
              ...
              <code className="truncate rounded bg-slate-950 px-1.5 py-0.5 font-mono text-[10px] text-slate-500">
          ...
          <Card key={String(row.id)} className="!p-4">
            <div className="flex items-start gap-4">
              <div className="w-14 shrink-0 text-center">
                ...
                <p className="text-[10px] text-slate-600">out of 10</p>
              </div>
  ...
  return (
    <div className="space-y-6">
      <Card className="space-y-2">
        ...
        <p className="text-sm leading-relaxed text-slate-400">{DATABASE_COPY.intro}</p>
      </Card>
          ...
          return (
            <button
              ...
              className={`rounded-xl border p-3 text-left transition-colors ${
                active ? "border-blue-500 bg-blue-950/40" : "border-slate-800 bg-slate-900/60 hover:border-slate-700"
              }`}
            >
              <p className={`text-sm font-medium ${active ? "text-blue-200" : "text-slate-200"}`}>
      ...
      <Card className="space-y-4">
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div className="space-y-1">
            <h3 className="font-medium text-slate-100">{info.label}</h3>
        ...
        <form onSubmit={submitSearch} className="flex gap-2">
          ...
          {query && (
            <button
              ...
              className="shrink-0 rounded-lg border border-slate-700 px-3 text-sm text-slate-400 transition-colors hover:text-slate-200"
            >
        ...
        <p className="rounded-lg border border-violet-900/50 bg-violet-950/20 px-3 py-2 text-xs leading-relaxed text-violet-200/80">
      ...
      {!loading && !pending && data && data.rows.length > 0 && (
        <div className="animate-fade-in-up space-y-4">
          {query && <p className="text-xs text-slate-500">{DATABASE_COPY.searching(query)}</p>}
          ...
          {page && (page.offset > 0 || page.has_more) && (
            <div className="flex items-center justify-between gap-3 border-t border-slate-800 pt-3">
              <button
                ...
                className="rounded-lg border border-slate-700 px-3 py-1.5 text-xs text-slate-400 transition-colors hover:text-slate-200 disabled:cursor-not-allowed disabled:opacity-30"
              >
