...
function StepLine({ text, state }: { text: string; state: "past" | "current" | "failed" }) {
  return (
    <li className="flex items-start gap-2">
      ...
      <span className={state === "past" ? "text-slate-500" : "text-slate-300"}>{text}</span>
    </li>
  );
}
...
function Item({ item }: { item: Activity }) {
  ...
  return (
    <div className="animate-fade-in-up rounded-xl border border-slate-800 bg-slate-900/95 p-3 shadow-xl shadow-black/40 backdrop-blur">
      <div className="flex items-start justify-between gap-2">
        ...
        <button
          ...
          aria-label="Dismiss"
          className="shrink-0 rounded px-1 text-slate-600 transition-colors hover:text-slate-300"
        >
          ✕
        </button>
      </div>
      ...
      {item.outcome && (
        <p
          className={`mt-2 border-t border-slate-800 pt-2 text-xs leading-relaxed ${
